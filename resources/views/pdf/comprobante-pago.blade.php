...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .resumen-neto .resumen-label, .resumen-neto .resumen-valor { font-size: 13px; font-weight: 800; color: #1a56db; border-top: 1px solid #e2e8f0; padding-top: 8px; }
...
<div class="page">
    ...
    <table class="detalle">
        ...
        <tbody>
            @foreach ($datos['pagosNomina'] as $fila)
                <tr>
                    <td>{{ $fila->movimiento->concepto ?? '—' }}</td>
                    <td>{{ $fila->movimiento?->torneo?->nombreTorneo ?? '—' }}</td>
    ...
    @if ($datos['deudasCompensadas']->isNotEmpty())
        <div class="seccion-titulo">Deudas compensadas (descontadas del pago)</div>
        <table class="detalle">
            <thead>
                <tr>
                    <th>Concepto</th>
            ...
            <tbody>
                @foreach ($datos['deudasCompensadas'] as $abono)
                    <tr>
                        <td>{{ $abono->movimiento->concepto ?? '—' }}</td>
                        <td class="num">${{ number_format((float) $abono->monto, 0, ',', '.') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div class="resumen">
        <div class="resumen-row">
            <span class="resumen-label">Total nómina saldada</span>
            <span class="resumen-valor">${{ number_format($datos['totalNomina'], 0, ',', '.') }}</span>
        </div>
        <div class="resumen-row">
            <span class="resumen-label">Deudas compensadas</span>
            <span class="resumen-valor">− ${{ number_format($datos['totalDeudas'], 0, ',', '.') }}</span>
        </div>
        <div class="resumen-row resumen-neto">
            <span class="resumen-label">Neto desembolsado</span>
            <span class="resumen-valor">${{ number_format($datos['netoDesembolsado'], 0, ',', '.') }}</span>
        </div>
    </div>

    <div class="firmas">
        <div class="firma-box">
            <div class="firma-linea">Entrega — Tesorería</div>
        </div>
        <div class="firma-box">
            <div class="firma-linea">Recibe — Árbitro</div>
        </div>
    </div>
